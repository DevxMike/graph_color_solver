...
public:
	...
	friend std::ostream& operator<<(std::ostream& os, simpleGA<TargetType, IndividualType>& algo) {
		os << "Population size: " << simpleGA_defaults::population_size << std::endl;
		os << "Generation count: " << algo.get_epoch() << std::endl;

		const auto& best = algo.get_best();
